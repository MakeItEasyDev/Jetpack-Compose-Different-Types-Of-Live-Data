...
class LaunchFlowLiveDataViewModel(
    private val savedStateHandle: SavedStateHandle
): ViewModel() {
    private val _liveDataPost = SafeMutableLiveData(savedStateHandle, "LivePostKey", 0)
    val liveDataPost: LiveData<Int> = _liveDataPost

    private val _liveDataSetWithContext = SafeMutableLiveData(savedStateHandle, "LiveSetWithContextKey", 0)
    val liveDataSetWithContext: LiveData<Int> = _liveDataSetWithContext

    private val _liveDataSetLaunch = SafeMutableLiveData(savedStateHandle, "LiveSetLaunchKey", 0)
    val liveDataSetLaunch: LiveData<Int> = _liveDataSetLaunch

    private val _stateFlow = SaveableMutableStateFlow(savedStateHandle, "FlowKey", 0)
